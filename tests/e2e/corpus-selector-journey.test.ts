import { expect, test, setupWebLLMProvider, navigateAndWaitForHydration } from '../utils/test-helpers-consolidated';
...
test.describe('Corpus Selector User Journey (Isolated) (Mocked)', () => {
  ...
  test('corpus selector is visible in chat interface', async ({ page }) => {
    ...
    await expect(corpusSelector).toBeVisible({ timeout: 5000 });
  ...
  test('can open corpus selector dropdown', async ({ page }) => {
    ...
    const areaTab = page.locator('[data-testid="corpus-tab-areas"]');
    await expect(areaTab).toBeVisible();
    
    const documentTab = page.locator('[data-testid="corpus-tab-documents"]'); 
  ...
  test('can switch between area and document tabs', async ({ page }) => {
    ...
    const searchInput = page.locator('[data-testid="corpus-search"]');
  ...
  test('can search for documents', async ({ page }) => {
    ...
    await smartWait(page, "interaction"); // Wait for search debounce
