...
import { setupPage, navigateAndWaitForHydration, toggleDarkMode, isVisibleInDarkMode, setupWebLLMProvider, testDarkMode, clearAllStorage, waitForHydration } from '../utils/test-helpers';
...
test.describe('Dark Mode User Journey - Isolated', () => {
  ...
  test('chat interface displays correctly in dark mode', async ({ page }) => {
    ...
    const chatInterface = page.locator('[data-testid={${TEST_IDS.chat.container}}]');
    await expect(chatInterface).toBeVisible();
    await expect(chatInterface).toHaveCSS('background-color', 'rgb(17, 24, 39)'); // gray-900
    
    // Check input area has dark styling
    const inputArea = page.locator('[data-testid={${TEST_IDS.chat.input}}]');
  ...
  test('case manager displays correctly in dark mode', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid={${TEST_IDS.cases.container}}]')).toHaveCSS('background-color', 'rgb(17, 24, 39)'); // gray-900
    
    // Check sidebar has proper dark styling
    const sidebar = page.locator('[data-testid={${TEST_IDS.cases.container}}] > div').first();
  ...
  test('theme preference persists across different pages', async ({ page }) => {
    ...
    await waitForHydration(page);await expect(page.locator('html')).toHaveClass(/dark/);
