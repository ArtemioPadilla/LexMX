...
import { setupPage, navigateAndWaitForHydration, clearAllStorage, setupWebLLMProvider, waitForHydration } from '../utils/test-helpers';
...
test.describe('Language Switching - Isolated', () => {
  ...
  test('language selector is visible and functional', async ({ page }) => {
    await page.goto('/');
    await waitForHydration(page);// Check language selector is visible using data-testid
  ...
  test('homepage content changes with language', async ({ page }) => {
    await page.goto('/');
    await waitForHydration(page);// Verify Spanish content is displayed by default using data-i18n
  ...
  test('chat interface translates correctly', async ({ page }) => {
    ...
    const welcomeFallback = page.locator('[data-testid={${TEST_IDS.chat.container}}]').locator('text=//Bienvenido|Welcome/i/i');
  ...
  test('navigation menu translates correctly', async ({ page }) => {
    await page.goto('/');
    await waitForHydration(page);// Check Spanish navigation
  ...
  test('language preference persists across pages', async ({ page }) => {
    ...
    await page.goto('/');
    await waitForHydration(page);const langSelector = page.locator('.language-selector button').first();
    await langSelector.click();
    await page.click('.language-selector button:has-text("English")');
    
    // Navigate to chat
    await page.goto('/chat');
    await waitForHydration(page);await expect(page.locator('.language-selector button').first()).toContainText('EN');
  ...
  test('example questions translate in chat', async ({ page }) => {
    ...
    const spanishExample = page.locator('text=//¿Qué dice el artículo 123 constitucional/i/i');
    ...
    const englishExample = page.locator('text=//What does constitutional article 123/i/i');
  ...
  test('error messages translate correctly', async ({ page }) => {
    ...
    const spanishError = page.locator('text=//No tienes proveedores/i/i');
    ...
    const englishError = page.locator('text=//No AI providers configured/i/i');
    await expect(englishError).toBeVisible();
  });

  test('language selector works in dark mode', async ({ page }) => {
    await page.goto('/');
    await waitForHydration(page);// Enable dark mode first
  ...
  test('language and theme preferences persist together', async ({ page }) => {
    await page.goto('/');
    await waitForHydration(page);// Set dark mode and English
    const themeToggle = page.locator('.theme-toggle button').first();
    await themeToggle.click();
    await page.click('.theme-toggle button:has-text("Oscuro")');
  ...
  test('language selector keyboard navigation works', async ({ page }) => {
    await page.goto('/');
    await waitForHydration(page);// Focus on language selector
    const langSelector = page.locator('.language-selector button').first();
