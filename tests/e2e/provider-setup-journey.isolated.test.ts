...
test.describe('Provider Setup to Chat Journey - Isolated', () => {
  ...
  test('complete provider setup flow and use chat', async ({ page }) => {
    ...
    await waitForHydration(page);// 2. Wait for setup wizard to load
    ...
    await page.waitForSelector('[data-testid={${TEST_IDS.chat.container}}]', { state: 'visible' });
    ...
    const textarea = page.locator('[data-testid={${TEST_IDS.chat.input}}], textarea[placeholder*="legal query"]');
    ...
    await expect(page.locator('text=//¿Qué es el artículo 123 constitucional?/i/i')).toBeVisible();
    
    // 17. Verify processing started (loading indicator or processing message)
    const processingIndicator = page.locator('.animate-spin').or(page.locator('text=//Analizando tu consulta legal/i/i'));
  ...
  test('persists provider configuration after page refresh', async ({ page }) => {
    ...
    await expect(page.locator('text=//Test message after refresh/i/i')).toBeVisible();
  });
  ...
  test('redirects to setup if no providers configured', async ({ page }) => {
    ...
    await waitForHydration(page);// 3. Should show warning message about no providers
    await page.waitForSelector('[data-testid={${TEST_IDS.chat.container}}]', { state: 'visible' });
  ...
  test('supports multiple provider configuration', async ({ page }) => {
    ...
    await page.click('button[aria-label="Enviar mensaje"], button[aria-label="Send message"]');
    await expect(page.locator('text=//Query with multiple providers/i/i')).toBeVisible();
  });
  ...
  test('handles provider configuration errors gracefully', async ({ page }) => {
    ...
    const errorText = page.locator('text=//formato|inválido|error/i/i');
