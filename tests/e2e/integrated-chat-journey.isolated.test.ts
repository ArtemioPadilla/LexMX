...
import { setupPage, navigateAndWaitForHydration, setupWebLLMProvider, setupAllMockProviders, setupLegacyMockProviders, createTestCase, toggleDarkMode, clearAllStorage, isVisibleInDarkMode, waitForHydration } from '../utils/test-helpers';
import { TEST_IDS } from '../../src/utils/test-ids';
import { TEST_DATA } from '../../src/utils/test-data';

test.describe('Integrated Chat Journey with All Features - Isolated', () => {
  ...
  test('complete user journey from setup to chat with all features', async ({ page }) => {
    ...
    await page.goto('/');
    await waitForHydration(page);await page.waitForTimeout(500);
    ...
    await waitForHydration(page);await page.waitForURL('**/chat');
  ...
  test('chat with dark mode and provider switching', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid={${TEST_IDS.provider.selectorToggle}}]', { timeout: 10000 }).catch(() => {});
    await page.waitForSelector('[data-testid={${TEST_IDS.corpus.selectorToggle}}]', { timeout: 10000 }).catch(() => {});
    
    // Check if they exist and are visible
    const providerSelector = page.locator('[data-testid={${TEST_IDS.provider.selectorToggle}}]').first();
    const corpusSelector = page.locator('[data-testid={${TEST_IDS.corpus.selectorToggle}}]').first();
    
    if (await providerSelector.isVisible()) {
      const providerVisible = await isVisibleInDarkMode(page, '[data-testid={${TEST_IDS.provider.selectorToggle}}]');
      expect(providerVisible).toBe(true);
    }
    
    if (await corpusSelector.isVisible()) {
      const corpusVisible = await isVisibleInDarkMode(page, '[data-testid={${TEST_IDS.corpus.selectorToggle}}]');
      expect(corpusVisible).toBe(true);
    }
    
    // Switch provider
    const providerBtn = page.locator('[data-testid={${TEST_IDS.provider.selectorToggle}}]').first();
    ...
    await expect(page.locator('text=//Test query in dark mode with OpenAI/i/i')).toBeVisible();
  });
  ...
  test('create case and link to chat conversation', async ({ page }) => {
    ...
    await expect(page.locator('text=//Consulta realizada sobre requisitos de divorcio/i/i')).toBeVisible();
  });
  ...
  test('mobile responsive workflow', async ({ page }) => {
    ...
    await waitForHydration(page);await page.waitForURL('**/casos');
  ...
  test('error handling and edge cases', async ({ page }) => {
    ...
    const errorMessage = page.locator('text=//No tienes proveedores|No AI providers configured|Configuración/i/i');
    ...
    await page.goto('/casos');
    await waitForHydration(page);await page.click('[data-testid={${TEST_IDS.cases.newCaseButton}}]');
