...
import { setupPage, navigateAndWaitForHydration, clearAllStorage, waitForHydration } from '../utils/test-helpers';
import { TEST_IDS } from '../../src/utils/test-ids';
import { TEST_DATA } from '../../src/utils/test-data';

test.describe('WebLLM Integration Flow - Isolated', () => {
  ...
  test('complete WebLLM setup and usage flow', async ({ page }) => {
    ...
    // 8. Complete setup
    await page.waitForSelector('text=//Configuración Completa|Setup Complete/i/i', { timeout: 10000 });
    ...
    // 10. Verify no errors
    const errorMessages = page.locator('text=//error|Error/i/i');
    const errorCount = await errorMessages.count();
    
    // Check for specific WebLLM errors
    const webllmErrors = page.locator('text=//WebLLM.*error|Cannot read properties.*find/i/i');
  ...
  test('WebLLM handles WebGPU not available gracefully', async ({ page }) => {
    ...
    await expect(page.locator('text=//WebGPU.*not supported|navegador compatible|Chrome.*Edge/i/i')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('WebLLM error messages are user-friendly', async ({ page }) => {
    ...
    const errorMessage = page.locator('text=//error|Error|problema|intenta nuevamente/i/i').first();
