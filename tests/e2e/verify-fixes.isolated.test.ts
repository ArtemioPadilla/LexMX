...
test('verify hydration and service worker fixes', async ({ page }) => {
  ...
  await page.waitForSelector('[data-testid={${TEST_IDS.chat.container}}]', { state: 'visible', timeout: 10000 });
...
test('verify provider setup to chat flow works', async ({ page }) => {
  ...
  await expect(page.locator('[data-testid={${TEST_IDS.chat.container}}]')).toBeVisible();
});