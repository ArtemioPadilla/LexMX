...
import {
  waitForHydration,
  navigateWithHydration,
  setupWebLLMWithTestIds,
  selectCorpusWithTestIds,
  createCaseWithTestIds,
  sendChatMessageWithTestIds,
  toggleThemeWithTestIds,
  switchLanguageWithTestIds,
  clearAndReset,
  setupAllMockProvidersWithTestData,
...
test.describe('Integrated Chat Journey with All Features', () => {
  ...
  test('chat with dark mode and provider switching', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="provider-selector-toggle"]', { timeout: 10000 }).catch(() => {});
    await page.waitForSelector('[data-testid="corpus-selector-toggle"]', { timeout: 10000 }).catch(() => {});
    
    // Check if they exist and are visible
    const providerSelector = page.locator('[data-testid="provider-selector-toggle"]').first();
    const corpusSelector = page.locator('[data-testid="corpus-selector-toggle"]').first();
    
    if (await providerSelector.isVisible()) {
      const providerVisible = await isVisibleInDarkMode(page, '[data-testid="provider-selector-toggle"]');
      expect(providerVisible).toBe(true);
    }
    
    if (await corpusSelector.isVisible()) {
      const corpusVisible = await isVisibleInDarkMode(page, '[data-testid="corpus-selector-toggle"]');
      expect(corpusVisible).toBe(true);
    }
    
    // Switch provider
    const providerBtn = page.locator('[data-testid="provider-selector-toggle"]').first();
