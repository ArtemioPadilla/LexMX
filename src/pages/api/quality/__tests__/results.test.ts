...
`;
      // Create a proper Blob mock that will pass instanceof check
      const mockBlob = Object.create(Blob.prototype);
      mockBlob.text = vi.fn().mockResolvedValue(report);
      mockBlob.type = 'text/markdown';
      return mockBlob;
    })
  };
...
// Also set on globalThis for API access
Object.defineProperty(globalThis, 'window', {
  value: window,
  configurable: true
});

// Ensure localStorage is available on globalThis for the API to find
...
describe('Quality Results API Endpoint', () => {
  ...
  describe('GET /api/quality/results', () => {
    ...
    describe('Format Options', () => {
      it('should return summary format', async () => {
        // Restore original mock data after previous test override
        const { qualityTestSuite } = await import('../../../../lib/admin/quality-test-suite');
        const mockStoredResults = [
          {
            suiteName: 'Mexican Legal Quality Suite',
        ...
        qualityTestSuite.getStoredResults = vi.fn().mockReturnValue(mockStoredResults);
    ...
    describe('Error Handling', () => {
      ...
      it('should handle invalid limit parameter', async () => {
        // Restore normal mock after error test
        const { qualityTestSuite } = await import('../../../../lib/admin/quality-test-suite');
        const mockStoredResults = [
          { suiteName: 'Mexican Legal Quality Suite', totalTests: 15, passedTests: 12, averageScore: 0.82, totalDuration: 18000, results: [], timestamp: Date.now() },
          { suiteName: 'Mexican Legal Quality Suite', totalTests: 15, passedTests: 10, averageScore: 0.78, totalDuration: 19500, results: [], timestamp: Date.now() - 86400000 },
          { suiteName: 'Mexican Legal Quality Suite', totalTests: 12, passedTests: 8, averageScore: 0.75, totalDuration: 15000, results: [], timestamp: Date.now() - 172800000 }
        ];
        qualityTestSuite.getStoredResults = vi.fn().mockReturnValue(mockStoredResults);
  ...
  describe('DELETE /api/quality/results', () => {
    ...
    it('should clear stored test results', async () => {
      ...
      // Verify that results are actually cleared by checking the qualityTestSuite
      const { qualityTestSuite } = await import('../../../../lib/admin/quality-test-suite');
      // After DELETE, getStoredResults should be called again, so we need to mock it to return empty
      qualityTestSuite.getStoredResults = vi.fn().mockReturnValue([]);
      
      // Test that subsequent GET request returns empty results
      const getResponse = await GET(mockContext);
      const getData = await getResponse.json();
      expect(getData.data.totalAvailable).toBe(0);
    });
  ...
  describe('POST /api/quality/results', () => {
    describe('Export Latest Operation', () => {
      ...
      it('should export latest test results', async () => {
        // Restore normal mock for export test
        const { qualityTestSuite } = await import('../../../../lib/admin/quality-test-suite');
        const mockStoredResults = [
          {
            suiteName: 'Mexican Legal Quality Suite',
