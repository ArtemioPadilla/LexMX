...
| 4 Chat y configuración | ✅ | Kit UI de Inceptor instalado; `/chat` sobre ChatThread/PromptInput/CitationList/AIOutputLabel/AIFeedback con modos Pregunta/Investiga, `grounded` y enlaces oficiales; `/setup` como wizard Stepper + Form + zod; `useTranslation` sobre `useSyncExternalStore`; humo en Chromium. Lighthouse CI (`npm run lighthouse`, `lighthouserc.json` con presupuestos por ruta y auditorías a11y como aserciones): accesibilidad 100 en `/`, `/chat` y `/wiki`; rendimiento 84 / 73 / 85 (LCP 2.8 s / 4.3 s / 2.6 s); `/chat` bajó de 463 KB a 283 KB de script en carga (presupuesto 300 KB) al cargar transformers.js solo al enfocar el prompt y el selector de modelo, la recomendación de proveedor y el progreso de WebLLM bajo demanda; corregidos en el camino un error de hidratación #418 causado por el minificador de HTML, rutas del manifest PWA sin barra y contrastes < 4.5:1 |
...
| 6 Corpus real | ✅ (MX, CL) | Importador desde LegalIA (`src/pipeline/legalia.ts`, `scripts/corpus/*`) probado con la LFT real; `corpus-update.yml` publica el release `corpus-latest` y `deploy.yml` lo consume; embeddings por documento (layout 2.1) y `CorpusInstaller` en el cliente: instala por shard, reanuda, omite la red si la versión ya está en IndexedDB y reporta progreso en `$corpusInstall`; oleada 1 ampliada a 39 leyes federales (`DEFAULT_SLUGS`) importadas sin fallos con el parser corregido; segunda oleada iniciada: adaptador Chile `src/pipeline/leychile.ts` (XML de LeyChile → mismo `LegalDocument`, con tests) y `npm run corpus:import:cl` (13 normas: constitución, códigos, leyes de consumo, datos y sociedades); corrido contra LeyChile: 11 normas, 9 614 chunks de artículo (partes ≤ 1 800 caracteres), embeddings con el mismo modelo; benchmark `evals/cl-nacional` (24 casos): documento recall@5 = 1.00, artículo recall@5 = 0.79 con re-ranking léxico, línea base 0.79; el cliente instala el corpus chileno bajo demanda al elegir la jurisdicción (`corpus/cl/`, claves `corpusVersion:cl`; verificado en Chromium: 11 documentos y 9 614 chunks en 9 s junto al corpus mexicano, sin borrar nada de éste) y `corpus-cl-update.yml` lo publica semanalmente como release `corpus-cl` |
| 7 Modo grounded y evaluación | ✅ | Chat muestra `grounded` y fuentes con enlace oficial; benchmark de recuperación (`evals/`, 43 casos) sobre las 39 leyes: documento recall@5 = 0.98, artículo recall@5 = 0.81, recall@10 = 0.93 tras (a) indexar solo artículos (los títulos/capítulos desplazaban artículos reales) y (b) penalizar transitorios ×0.92; reglas compartidas en `src/lib/rag/ranking.ts` y el eval; después, re-ranking léxico (solapamiento de términos + número de artículo mencionado) sobre 60 candidatos: artículo recall@5 0.81 → 0.88 (México) y 0.71 → 0.79 (Chile), líneas base 0.88 / 0.79. Corregido: el motor buscaba en un índice en memoria vacío cuando el corpus real estaba instalado (siempre `grounded=false`); ahora consulta el vector store con el prefijo `query:` de e5 |
| 8 Herramientas sobre documentos | ✅ | Comparación en navegador (`/comparar`, diff por línea y palabra, «Explicar cambios en el chat» prepara el prompt con el diff), revisión tabular (`/tablas`: N documentos → campos definidos por el usuario (partes, fecha, monto, cláusula, palabra clave, regex) extraídos de forma determinista con contexto, tabla editable y CSV), calculadoras laborales y fiscales puras con tests (`src/jurisdictions/mx/calculators.ts`, `calculators-fiscal.ts`: finiquito, ISR mensual, IVA, recargos, plazos hábiles), expedientes en IndexedDB (`LexMX_Cases`) que reciben comparaciones, tablas y transcripciones con «Guardar en expediente» (`src/components/SaveToCase.tsx`) |
| 9 Evaluación abierta y confianza | ✅ | `evals/` con 43 casos, `npm run eval:retrieval` corre en `corpus-update.yml` antes de publicar y falla bajo `evals/baseline.json` (0.813); `/seguridad`; LexMX Escritorio (Tauri 2). Pendiente de tracción: precisión de cita con modelo (requiere proveedor en CI) |
| 11.2 Jurisdicción | ✅ | `src/jurisdictions/` en estricto con tests: contrato (`types.ts`), México como referencia (entidades, jerarquía, fuentes, citas, calculadoras, plantillas, marco de privacidad) y módulos para CL, AR, CO, PE, BR, UY, EC, CR y PA; `LegalDocument.jurisdiction` cableado en el corpus, el instalador y el filtro del motor; selector en chat, plantillas, calculadoras y organizaciones; `docs/CONTRIBUTING-JURISDICTIONS.md`. Pendiente por país: corpus (línea B) |
| 11.4 C Producto local-first | ✅ | Chat con cita verificable y modo grounded, modos Pregunta/Investiga, filtro jurisdiccional, expedientes en IndexedDB, calculadora laboral MX (finiquito/liquidación) en `/herramientas`, Biblioteca (`/biblioteca`: instalado, tamaño, fecha, actualizar/borrar), novedades desde la última visita en la portada (fechas de reforma del corpus), plantillas por jurisdicción en `/plantillas` (contrato individual de trabajo, carta poder, solicitud de acceso a la información; campos → escrito anclado a artículos, exportación Markdown/Word (.docx)/PDF por impresión, todo en el navegador), comparación de documentos en `/comparar` (diff por línea y palabra, unificado o lado a lado, carga TXT/PDF/DOCX, copia como diff unificado), OCR en el dispositivo con Tesseract.js (`/herramientas` y automático en el extractor de PDF cuando una página no trae capa de texto) y dictado por voz en el chat (SpeechRecognition del navegador cuando existe; si no, grabación en el dispositivo y Whisper local); notas de voz y transcripción local con marcas de tiempo en `/herramientas` (Whisper tiny/base/small vía transformers.js, micrófono o archivo, .txt). Las transcripciones, comparaciones y tablas se guardan en el expediente («Guardar en expediente») |
| 11.4 E Canales | ✅ | Servidor MCP local `scripts/mcp/server.ts` (`search_corpus`, `get_article`, `list_documents` sobre stdio, sin dependencias) probado contra el corpus de 14 leyes; PWA con Workbox; LexMX Escritorio (Tauri 2, `src-tauri/`, `desktop.yml`); complemento de Word (`public/office/manifest.xml` + `/office/taskpane`: el mismo chat como panel de tareas con «Insertar última respuesta» vía Office.js, con o sin citas; sin servidor); móvil: scripts `mobile:android:*` / `mobile:ios:*` y `docs/MOBILE.md` (mismo `dist/`, decisiones de modelos y distribución). `mobile.yml` genera el proyecto Android con los SDKs en CI y produce el APK de depuración, e iOS compila sin firma en macOS; la firma para tiendas espera los secretos del repositorio (`docs/MOBILE.md`) |
| 11.4 G Confianza | ✅ | `/seguridad`, marco de privacidad por país en cada módulo, aviso en cada respuesta, `docs/CONTRIBUTING-JURISDICTIONS.md`; `docs/SECURITY-PROGRAM.md` (mapa de controles a ISO 27001 Anexo A y alineación con ISO 42001, con lo que ya es código y lo pendiente) y `docs/COMMUNITY.md` (despachos aliados y clínicas/estudiantes: qué reciben, qué aportan, reglas). Fuera del repositorio: la auditoría externa (ruta en `docs/SECURITY-PROGRAM.md`) y la aportación de casos por aliados (`docs/COMMUNITY.md`) |
| 11.9 Supabase | ✅ | `supabase/` (6 migraciones con RLS, 4 Edge Functions, `supabase.yml`, test de invariantes); cliente guardado `src/lib/supabase.ts`, `$session`/`$guardUser`, `route-guard.tsx`, `/cuenta` con alta e inicio de sesión (modo local si no hay variables); pestañas Organizaciones (crear, integrantes, roles por allowlist, invitaciones con token hasheado + Edge Function `accept-invite`), Plan y consumo (suscripción y `usage` del mes, solo lectura, cuotas por plan) y Monitoreo (expedientes vigilados por adaptador, leyes seguidas, notificaciones) sobre `src/lib/server/*` con tests contra un cliente falso. Directorio verificado de abogados y canalización en `/abogados` (`src/lib/server/lawyers.ts` con tests; solo perfiles verificados, resumen sin expediente, perfil propio). Portal de pago: Edge Function `billing-portal` (Stripe Checkout para Pro/Team y Customer Portal; botones en Plan y consumo); monitoreo programado: `0007_cron_monitoring.sql` (pg_cron + pg_net + Vault) llama cada hora a `court-monitor`, que corre los adaptadores de `src/lib/monitoring/adapters.ts` (DOF diario, SCJN y CJF por RSS; parsing y matching puros con tests) y escribe `court_events` deduplicados. Los adaptadores estatales se añaden en `ADAPTERS` (contrato `feedUrl`/`parse`) y los feeds oficiales se verifican al desplegar el proyecto Supabase |

Lo que no es código (secretos de tiendas y Supabase, primer despliegue de los
releases de corpus, auditoría externa, programa de comunidad) queda rastreado
en el issue #74 con la mecánica del propio plan: issue → PR → merge → deploy.
...
casos obsoletos → 557 tras la ola 2 → 668 (todos verdes, 83 archivos).
...
| Captura automática de acuerdos de tribunales y boletines | D | ✅ servidor | `court-monitor` por pg_cron con adaptadores DOF/SCJN/CJF; sin servidor no hay captura |
...
| Complemento de Word | E | ✅ | `public/office/manifest.xml` + `/office/taskpane`, mismo deploy |
| Apps iOS/Android | E | ✅ CI | `mobile.yml` (APK de depuración, iOS sin firma); firma de tiendas pendiente de secretos |
| Canalización a abogados, equipos, facturación | D | ✅ servidor | `/abogados` (directorio verificado por materia y entidad, canalizaciones con resumen, perfil del abogado), organizaciones y facturación en `/cuenta`; todo sobre Supabase con RLS |
