...
| 6 Corpus real | ✅ (MX) · 🔄 (CL) | Importador desde LegalIA (`src/pipeline/legalia.ts`, `scripts/corpus/*`) probado con la LFT real; `corpus-update.yml` publica el release `corpus-latest` y `deploy.yml` lo consume; embeddings por documento (layout 2.1) y `CorpusInstaller` en el cliente: instala por shard, reanuda, omite la red si la versión ya está en IndexedDB y reporta progreso en `$corpusInstall`; oleada 1 ampliada a 39 leyes federales (`DEFAULT_SLUGS`) importadas sin fallos con el parser corregido; segunda oleada iniciada: adaptador Chile `src/pipeline/leychile.ts` (XML de LeyChile → mismo `LegalDocument`, con tests) y `npm run corpus:import:cl` (13 normas: constitución, códigos, leyes de consumo, datos y sociedades); pendiente correrlo con red hacia bcn.cl y publicar `corpus-cl` |
